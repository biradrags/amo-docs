...
*   Виджет не должен влиять на данные в глобальной переменной AMOCRM, он может только читать данные оттуда. Список допустимых обращений описан в статье ["Переменные окружения"](/developers/content/web_sdk/env_variables). Мы не гарантируем работу недокументированных переменных окружения в будущем, поэтому настоятельно рекомендуем отказаться от их использования.
