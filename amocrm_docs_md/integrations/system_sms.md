...
Чтобы существующий или новый виджет начал поддерживать функционал системных SMS, вам необходимо добавить новый объект sms и указать дополнительный location sms в [файле manifest.json](/integrations/structure#manifest.html).
...
| token | string | JWT токен, который позволит идентифицировать аккаунт, из которого сделан запрос. Используется токен, описание которого доступно в [статье](/oauth/disposable-tokens.html). Обязательно валидируйте подпись полученного токена, это позволит быть уверенными, что запрос пришел из amoCRM. |
