...
[Подробнее о работе с элементами каталога через API](/catalogs/catalogs.html)
...
[Подробнее о работе с кастомными полями через API](/crm_platform/custom-fields#custom-fields-edit.html)

#### Кастомизация карточки элемента списка

Интеграцию могут заменить стандартную карточку редактирования элемента списка на собственную. Для этого необходимо указать специальную [область подключения](/integrations/areas.html) "catalogs", указать карточку какого каталога будет заменять интеграция и реализовать специальный callback loadCatalogElement.

[Подробнее о кастомизации карточки элемента списка](/integrations/catalogs_sdk.html)