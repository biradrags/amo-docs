...
1.  Получить доступ к данным аккаунта в котором вы являетесь администратором "под собой", используя [упрощенную механику авторизации](/oauth/step-by-step#easy_auth.html), без использования редиректов. Для простых задач – простые решения.
2.  [Получить авторизацию](/oauth/step-by-step#get_auth_code.html) и [разрешения](/oauth/scopes.html) от пользователя amoCRM на доступ к данным его аккаунта в соответствии с запрошенными разрешениями.
3.  Регистрировать и авторизовывать пользователя в своем приложении на основе авторизации amoCRM, используя [Кнопку входа amoCRM](/oauth/button.html)
4.  Загружать виджет в систему для использования в браузере.

Чтобы облегчить процесс написания кода интеграции, мы разработали [библиотеку](/oauth/lib.html) для авторизации. На текущий момент мы поддерживаем только написанную на PHP библиотеку, но, нам кажется, что просмотр исходных файлов может подсказать решения и разработчикам на других языках.
...
[Возможности платформы](/platform/abilities.html)  
