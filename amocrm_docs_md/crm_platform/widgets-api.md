...
| Параметр | Тип данных | Описание |
| --- | --- | --- |
| data | array | Данные для виджета, в коде бота можно получить их по ключу \_\_\_DOUBLE\_OPEN\_BRACE\_\_\_json.НАЗВАНИЯ\_КЛЮЧА\_МАССИВА\_\_\_DOUBLE\_CLOSE\_BRACE\_\_\_, где НАЗВАНИЯ\_КЛЮЧА\_МАССИВА это имя поля переданного в data. Необязательный параметр. |
| execute\_handlers | array | На данный момент поддерживаются хендлеры следующих типов: show, goto. Необязательный параметр. |

#### Пример запроса

В примере передадим виджету поле status, в любом блоке после widget\_request по ключу \_\_\_DOUBLE\_OPEN\_BRACE\_\_\_json.status\_\_\_DOUBLE\_CLOSE\_BRACE\_\_\_ виджет сможет получить его значение: "success".  
