...
| entity\_id | int  
null | ID сущности, по которой ведется беседа |
| entity\_type | string  
null | Тип сущности, по которой ведется беседа (lead, customer) |
...
| missed\_at | int  
null | Дата когда беседа была пропущена (не отвечена за время, установленное в настройках аккаунта), передается в Unix Timestamp |
