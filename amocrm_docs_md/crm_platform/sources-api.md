...
Каждый источник обладает уникальным кодом (external\_id) и двух источников с одинаковым кодом быть не может.
...
| origin\_code | string  
null | Код основного канала источника. Данный канал чата будет использоваться при инициализации общения. Поле не является обязательным. |
...
| services\[0\]\[pages\]\[0\]\[link\] | string | Номер телефона, который будет указан в кнопке WhatsApp и которому будет писать клиент на странице сайта. В CRM Plugin ссылка будет сформирована следующим образов: [https://wa.me/{Указанный](https://wa.me/{Указанный) номер телефона} |
...
_POST /api/v4/website\_buttons_
...
_POST /api/v4/website\_buttons/{source\_id}/online\_chat_
...
_PATCH /api/v4/website\_buttons/{source\_id}_
...
_GET /api/v4/website\_buttons/{source\_id}_
