...
Для указания callback-функции нужно создать глобальный объект **AMOPIXEL\_IDENTIFIER\_PARAMS**, где в поле **“onload”** будет указана callback-функция.  
Первым аргументом в эту функцию будет передан сам объект **AMOPIXEL\_IDENTIFIER**, но Вы можете обратиться к нему и через глобальный объект window, т.е. **window.AMOPIXEL\_IDENTIFIER**.
